defmodule Ecto.Integration.MigrationTest do
  ...
  defmodule AddColumnMigration do
    ...
    def up do
      ...
      execute(
        "INSERT INTO add_col_migration (value, to_be_added) VALUES (1, 2)"
      )
    end
  ...
  defmodule AlterColumnMigration do
    ...
    def up do
      ...
      execute(
        "INSERT INTO alter_col_migration (from_null_to_not_null) VALUES ('foo')"
      )
    end
  ...
  defmodule AlterForeignKeyOnDeleteMigration do
    ...
    def up do
      ...
      alter table(:alter_fk_posts) do
        modify(
          :alter_fk_user_id,
          references(:alter_fk_users, on_delete: :nilify_all)
        )
      end
  ...
  defmodule AlterForeignKeyOnUpdateMigration do
    ...
    def up do
      ...
      alter table(:alter_fk_posts) do
        modify(
          :alter_fk_user_id,
          references(:alter_fk_users, on_update: :update_all)
        )
      end
      ...
      execute(
        "INSERT INTO alter_fk_posts (id, alter_fk_user_id) VALUES ('1', '1')"
      )
  ...
  defmodule DropColumnMigration do
    ...
    def up do
      ...
      execute(
        "INSERT INTO drop_col_migration (value, to_be_removed) VALUES (1, 2)"
      )
  ...
  test "create and drop table and indexes" do
    assert :ok == up(PoolRepo, 20_050_906_120_000, CreateMigration, log: false)

    assert :ok ==
             down(PoolRepo, 20_050_906_120_000, CreateMigration, log: false)
  end
  ...
  test "supports references" do
    ...
    assert :ok ==
             down(PoolRepo, 20_050_906_120_000, OnDeleteMigration, log: false)
  end
  ...
  test "create table if not exists and drop table if exists does not raise on failure" do
    assert :ok ==
             up(PoolRepo, 19_850_423_000_001, NoErrorTableMigration, log: false)
  end

  @tag :create_index_if_not_exists
  test "create index if not exists and drop index if exists does not raise on failure" do
    assert :ok ==
             up(PoolRepo, 19_850_423_000_002, NoErrorIndexMigration, log: false)
  end

  test "raises on NoSQL migrations" do
    assert_raise ArgumentError,
                 ~r"does not support keyword lists in :options",
                 fn ->
                   up(PoolRepo, 20_150_704_120_000, NoSQLMigration, log: false)
                 end
  end
  ...
  test "add column" do
    assert :ok ==
             up(PoolRepo, 20_070_906_120_000, AddColumnMigration, log: false)

    assert [2] ==
             PoolRepo.all(from(p in "add_col_migration", select: p.to_be_added))

    :ok = down(PoolRepo, 20_070_906_120_000, AddColumnMigration, log: false)
  end

  @tag :modify_column
  test "modify column" do
    assert :ok ==
             up(PoolRepo, 20_080_906_120_000, AlterColumnMigration, log: false)

    assert ["foo"] ==
             PoolRepo.all(
               from(p in "alter_col_migration", select: p.from_null_to_not_null)
             )

    assert [nil] ==
             PoolRepo.all(
               from(p in "alter_col_migration", select: p.from_not_null_to_null)
             )

    assert [nil] ==
             PoolRepo.all(
               from(
                 p in "alter_col_migration",
                 select: p.from_default_to_no_default
               )
             )
    ...
    assert [0] ==
             PoolRepo.all(
               from(
                 p in "alter_col_migration",
                 select: p.from_no_default_to_default
               )
             )

    query =
      "INSERT INTO alter_col_migration (from_not_null_to_null) VALUES ('foo')"

    assert catch_error(PoolRepo.query!(query))

    :ok = down(PoolRepo, 20_080_906_120_000, AlterColumnMigration, log: false)
  end
  ...
  test "modify foreign key's on_delete constraint" do
    assert :ok ==
             up(
               PoolRepo,
               20_130_802_170_000,
               AlterForeignKeyOnDeleteMigration,
               log: false
             )

    assert [nil] ==
             PoolRepo.all(
               from(p in "alter_fk_posts", select: p.alter_fk_user_id)
             )
  ...
  test "modify foreign key's on_update constraint" do
    assert :ok ==
             up(
               PoolRepo,
               20_130_802_170_000,
               AlterForeignKeyOnUpdateMigration,
               log: false
             )
  ...
  test "remove column" do
    assert :ok ==
             up(PoolRepo, 20_090_906_120_000, DropColumnMigration, log: false)

    assert catch_error(
             PoolRepo.all(
               from(p in "drop_col_migration", select: p.to_be_removed)
             )
           )

    :ok = down(PoolRepo, 20_090_906_120_000, DropColumnMigration, log: false)
  end

  @tag :rename_column
  test "rename column" do
    assert :ok ==
             up(PoolRepo, 20_150_718_120_000, RenameColumnMigration, log: false)

    assert [1] ==
             PoolRepo.all(
               from(p in "rename_col_migration", select: p.was_renamed)
             )

    :ok = down(PoolRepo, 20_150_718_120_000, RenameColumnMigration, log: false)
  end

  @tag :rename_table
  test "rename table" do
    assert :ok == up(PoolRepo, 20_150_712_120_000, RenameMigration, log: false)

    assert :ok ==
             down(PoolRepo, 20_150_712_120_000, RenameMigration, log: false)
  end

  @tag :prefix
  test "prefix" do
    assert :ok == up(PoolRepo, 20_151_012_120_000, PrefixMigration, log: false)

    assert :ok ==
             down(PoolRepo, 20_151_012_120_000, PrefixMigration, log: false)
  end
